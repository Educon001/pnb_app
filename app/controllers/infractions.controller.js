  ...
  createInfractionCON: async (_req, _res) => {
    ...
    try {
      ...
      if (!code || !name || !description || !article || !severity) {
        throw new CmmErrorClass(__filename, 'CPNB-CINFE001', 'Código, nombre, descripción, artículo y gravedad son obligatorios').returnValidate();
      }

      if (!taxUnits || !bolivarValue) {
        throw new CmmErrorClass(__filename, 'CPNB-CINFE002', 'Unidades tributarias y valor en bolívares son obligatorios').returnValidate();
      }

      const INFRACTION_RESULT = await createInfractionSV(_req.body, _req.police._id).catch((_error) => {
        throw new CmmErrorClass(__filename, 'CPNB-CINFE003').parseCatch(_error);
      });

      return CC_RESPONSE.send('Infracción creada exitosamente', INFRACTION_RESULT, 'INFRACTIONS001');
    } catch (_error) {
      return CC_RESPONSE.sendError(!_error.errorType ? new CmmErrorClass(__filename, 'CPNB-CINFE003', _error).server() : _error);
    }
  },
  ...
  getInfractionsCON: async (_req, _res) => {
    ...
    try {
      ...
      const RESULT = await getInfractionsSV(FILTERS, parseInt(page), parseInt(limit)).catch((_error) => {
        throw new CmmErrorClass(__filename, 'CPNB-CINFE004').parseCatch(_error);
      });

      return CC_RESPONSE.send('Infracciones obtenidas exitosamente', RESULT, 'INFRACTIONS002');
    } catch (_error) {
      return CC_RESPONSE.sendError(!_error.errorType ? new CmmErrorClass(__filename, 'CPNB-CINFE004', _error).server() : _error);
    }
  },
  ...
  getInfractionByIdCON: async (_req, _res) => {
    ...
    try {
      ...
      const INFRACTION_RESULT = await getInfractionByIdSV(id).catch((_error) => {
        throw new CmmErrorClass(__filename, 'CPNB-CINFE005').parseCatch(_error);
      });

      return CC_RESPONSE.send('Infracción obtenida exitosamente', INFRACTION_RESULT, 'INFRACTIONS003');
    } catch (_error) {
      return CC_RESPONSE.sendError(!_error.errorType ? new CmmErrorClass(__filename, 'CPNB-CINFE005', _error).server() : _error);
    }
  },
  ...
  updateInfractionCON: async (_req, _res) => {
    ...
    try {
      ...
      const INFRACTION_RESULT = await updateInfractionSV(id, UPDATE_DATA, _req.police._id).catch((_error) => {
        throw new CmmErrorClass(__filename, 'CPNB-CINFE006').parseCatch(_error);
      });

      return CC_RESPONSE.send('Infracción actualizada exitosamente', INFRACTION_RESULT, 'INFRACTIONS004');
    } catch (_error) {
      return CC_RESPONSE.sendError(!_error.errorType ? new CmmErrorClass(__filename, 'CPNB-CINFE006', _error).server() : _error);
    }
  },
  ...
  deleteInfractionCON: async (_req, _res) => {
    ...
    try {
      ...
      const RESULT = await deleteInfractionSV(id, _req.police._id).catch((_error) => {
        throw new CmmErrorClass(__filename, 'CPNB-CINFE007').parseCatch(_error);
      });

      return CC_RESPONSE.send(RESULT.message, null, 'INFRACTIONS005');
    } catch (_error) {
      return CC_RESPONSE.sendError(!_error.errorType ? new CmmErrorClass(__filename, 'CPNB-CINFE007', _error).server() : _error);
    }
  },
  ...
  getInfractionsBySeverityCON: async (_req, _res) => {
    ...
    try {
      ...
      const INFRACTIONS_RESULT = await getInfractionsBySeveritySV(severity).catch((_error) => {
        throw new CmmErrorClass(__filename, 'CPNB-CINFE008').parseCatch(_error);
      });

      return CC_RESPONSE.send('Infracciones obtenidas exitosamente', INFRACTIONS_RESULT, 'INFRACTIONS006');
    } catch (_error) {
      return CC_RESPONSE.sendError(!_error.errorType ? new CmmErrorClass(__filename, 'CPNB-CINFE008', _error).server() : _error);
    }
  },
  ...
  getInfractionsByVehicleTypeCON: async (_req, _res) => {
    ...
    try {
      ...
      const INFRACTIONS_RESULT = await getInfractionsByVehicleTypeSV(type).catch((_error) => {
        throw new CmmErrorClass(__filename, 'CPNB-CINFE009').parseCatch(_error);
      });

      return CC_RESPONSE.send('Infracciones obtenidas exitosamente', INFRACTIONS_RESULT, 'INFRACTIONS007');
    } catch (_error) {
      return CC_RESPONSE.sendError(!_error.errorType ? new CmmErrorClass(__filename, 'CPNB-CINFE009', _error).server() : _error);
    }
  },
  ...
  getInfractionsStatisticsCON: async (_req, _res) => {
    ...
    try {
      const STATISTICS_RESULT = await getInfractionsStatisticsSV().catch((_error) => {
        throw new CmmErrorClass(__filename, 'CPNB-CINFE010').parseCatch(_error);
      });

      return CC_RESPONSE.send('Estadísticas obtenidas exitosamente', STATISTICS_RESULT, 'INFRACTIONS008');
    } catch (_error) {
      return CC_RESPONSE.sendError(!_error.errorType ? new CmmErrorClass(__filename, 'CPNB-CINFE010', _error).server() : _error);
    }
  },
  ...
  searchInfractionsCON: async (_req, _res) => {
    ...
    try {
      ...
      if (!term || term.length < 2) {
        throw new CmmErrorClass(__filename, 'CPNB-CINFE011', 'El término de búsqueda debe tener al menos 2 caracteres').returnValidate();
      }

      const INFRACTIONS_RESULT = await searchInfractionsSV(term).catch((_error) => {
        throw new CmmErrorClass(__filename, 'CPNB-CINFE012').parseCatch(_error);
      });

      return CC_RESPONSE.send('Búsqueda completada exitosamente', INFRACTIONS_RESULT, 'INFRACTIONS009');
    } catch (_error) {
      return CC_RESPONSE.sendError(!_error.errorType ? new CmmErrorClass(__filename, 'CPNB-CINFE012', _error).server() : _error);
    }
  }
};
