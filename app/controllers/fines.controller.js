...
module.exports = {
  ...
  createFineCON: async (_req, _res) => {
    ...
    try {
      ...
      const FINE_RESULT = await createFineSV(
        VALIDATED_DATA,
        _req.police._id
      ).catch(_error => {
        console.log(_error);
  ...
  getFinesCON: async (_req, _res) => {
    ...
    try {
      ...
      if (
        !_req.police.roles.includes('admin') &&
        !_req.police.roles.includes('supervisor')
      ) {
        FILTERS.officer = _req.police._id;
      }

      const RESULT = await getFinesSV(
        FILTERS,
        parseInt(page),
        parseInt(limit)
      ).catch(_error => {
      ...
      if (FINE_RESULT.officer._id.toString() !== _req.police._id.toString()) {
        if (
          ...
          throw new CmmErrorClass(
            __filename,
            'FINEE004',
            'No tiene permisos para ver esta multa'
          ).frontend();
        }
      }

      return CC_RESPONSE.send(
        'Multa obtenida exitosamente',
        FINE_RESULT,
        'FINES003'
      );
    } catch (_error) {
  ...
  updateFineCON: async (_req, _res) => {
    ...
    try {
      ...
      const FINE_RESULT = await updateFineSV(
        id,
        UPDATE_DATA,
        _req.police._id
      ).catch(_error => {
      ...
      return CC_RESPONSE.send(
        'Multa actualizada exitosamente',
        FINE_RESULT,
        'FINES004'
      );
    } catch (_error) {
